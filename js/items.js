...
export const RARITY_ORDER = ['rare', 'epic', 'legendary'];

// 三個部位；bias 是該部位偏好的詞條 (權重加倍)，讓不同部位有性格
export const SLOTS = {
  goggles: { key: 'goggles', name: '特工墨鏡', icon: '🕶️', bias: ['dmg', 'cdr'] },
  coat:    { key: 'coat',    name: '特工風衣', icon: '🧥', bias: ['hp', 'magnet'] },
  boots:   { key: 'boots',   name: '特工皮鞋', icon: '👟', bias: ['speed', 'gold'] },
};
...
export const AFFIX_ORDER = ['dmg', 'hp', 'speed', 'magnet', 'cdr', 'gold'];
...
export function rollRarity(bonusWeight = 0) {
  return pickWeighted(RARITY_ORDER.map((k) => {
    const r = RARITIES[k];
    const boost = k === 'rare' ? 1 : k === 'epic' ? 1 + bonusWeight : 1 + bonusWeight * 3;
...
export function rollItem({ slot = null, rarity = null, ilvl = 1 } = {}) {
  ...
  const affixes = [];
  for (let i = 0; i < rarityDef.affixes && pool.length > 0; i++) {
